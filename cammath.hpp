...
template<typename T>
struct camera3 {
    Eigen::Vector2<T> size;
    T fov;
    Eigen::Quaternion<T> rotation;
    Eigen::Translation<T, 3> translation_inv;

    camera3(const Eigen::Vector2<T>& s, const T& f, Eigen::Vector3<T>& t, Eigen::Quaternion<T>& r) : size(s), fov(f), rotation(r), translation_inv(Eigen::Translation<T, 3>(t).inverse()) {}

    camera3(const Eigen::Vector2<T>&& s, const T&& f, Eigen::Vector3<T>&& t, Eigen::Quaternion<T>&& r) : size(std::move(s)), fov(std::move(f)), rotation(r), translation_inv(Eigen::Translation<T, 3>(t).inverse()) {}
